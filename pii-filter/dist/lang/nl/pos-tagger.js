 ...
 * @private
 */
export class DutchPOSTagger {
    constructor() {
        this.none_str = 'x';
        this.tagger = new Brill_POS_Tagger(new Lexicon('DU', this.none_str, this.none_str.toUpperCase()), new RuleSet('DU'));
    }
    /** @inheritdoc */
    tag(tokens, language_model) {
        let res = new Array();
        let intermediate_tokens = this.tagger.tag(tokens).taggedWords;
        ...
        class TagGroupAnnotator {
            ...
            annotate(items) {
                ...
                for (let i = 0; i < items.length; ++i) {
                    let c_group = this.current_group;
                    let [str, tag] = items[i];
                    ...
                    if (language_model.punctuation_map.has(str) &&
                        ...
                        if (str.match(/\r|\n/) ||
                            ...
                            let start_end_ok = this.first_uppercase && c_group.n_tags > 1;
                            if (start_end_ok)
                    ...
                    c_group.well_formed = Math.min(c_group.well_formed, 1.0);
                    tag.group = c_group;
                    c_group.n_tags++;
                }
                return items;
