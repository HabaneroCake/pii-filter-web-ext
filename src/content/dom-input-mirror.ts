...
{
    protected input_div:            HTMLDivElement;
    ...
    constructor(
        ...
        {
            this.input_div = this.document.createElement("div");
            this.div.appendChild(this.input_div);
        }
        else
            this.input_div = this.div;
            ...
            const update_position = (
                    ...
                    height != last_rect[3])
                {
                    ...
                    if (browser_name == 'Firefox')
                    {
                        ...
                        this.input_div.style.width =    computed_style.width;
                        this.input_div.style.height =   computed_style.height;
                    }
                    else
                    {
                        this.input_div.style.width =    `${ width }px`;
                        this.input_div.style.height =   `${ height }px`;
                    }
        ...
        Array.from(computed_style).forEach(
            key => this.input_div.style.setProperty(
                        key,
        ...
        switch (input_element.tagName)
        {
            ...
            case 'INPUT':
                this.input_div.style.cssText +=     'appearance: textfield;';
                this.input_div.style.overflow =     'hidden';
                //?
                // this.input_div.style.font =         '-moz-field';
                // this.input_div.style.font =         '-webkit-small-control';
                // whitespace
                this.input_div.style.whiteSpace =   'pre';
                bind_init(this.input_div, input_element as HTMLInputElement, '');
                break
            case 'TEXTAREA':
                this.input_div.style.cssText +=     'appearance: textarea;';
                // TODO: this should probably be handled on a case by case, not just auto
                this.input_div.style.overflow =     'auto';
                bind_init(this.input_div, input_element as HTMLTextAreaElement, '\0');
                break;
            default: break;
        }

        // remove margins
        this.input_div.style.margin =                   '0px';
        this.input_div.style.marginTop =                '0px';
        this.input_div.style.marginBottom =             '0px';
        this.input_div.style.marginLeft =               '0px';
        this.input_div.style.marginRight =              '0px';
        this.input_div.style.marginBlockStart =         '0px';
        this.input_div.style.marginBlockEnd =           '0px';
        this.input_div.style.marginInlineStart =        '0px';
        this.input_div.style.marginInlineEnd =          '0px';
        ...
        if (browser_name == 'Firefox')
        {
            ...
            this.input_div.style.padding =                  '0px';
            this.input_div.style.paddingLeft =              '0px';
            this.input_div.style.paddingRight =             '0px';
            this.input_div.style.paddingTop =               '0px';
            this.input_div.style.paddingBottom =            '0px';
            this.input_div.style.paddingBlockStart =        '0px';
            this.input_div.style.paddingBlockEnd =          '0px';
            this.input_div.style.paddingInlineStart =       '0px';
            this.input_div.style.paddingInlineEnd =         '0px';
        ...
        this.input_div.style.pointerEvents =            'none';
        this.div.style.pointerEvents =                  'none';
        this.input_div.style.display =                  'block';
        this.div.style.display =                        'block';
        // this.input_div.style.zIndex =                   '-99999';
        // this.input_div.style.opacity =                  '0.5';
        this.input_div.style.visibility =               'hidden';
    ...
    {
        if (this.input_div.childNodes.length == 0)
            return [];
        
        const text_len: number = this.input_div.childNodes[0].textContent.length;

        char_start_index =  Math.min(char_start_index, text_len - 1);
        char_end_index =    Math.min(char_end_index, text_len);
        
        let range: Range = document.createRange();
        range.setStart(this.input_div.childNodes[0], char_start_index);
        range.setEnd(this.input_div.childNodes[0], char_end_index);
        ...
        for (let rect of relative_rects)
        {
            ...
            const bounding_rect: DOMRect = this.input_div.getBoundingClientRect();
